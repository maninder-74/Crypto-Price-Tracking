    ...
    <>
      <div className="Home">
        <div className="hero">
          ...
          <p id="description">Welcome to the world's largest cryptocurrency marketplace. Sign upto explore more about cryptos.</p>

          <form onSubmit={searchHandler}>
            <input onChange={handleInput} list="coinlist" type="text"placeholder="Search Crypto.." required/>

            <datalist id="coinlist">
              {allCoinsData.map((item, index) => (<option key={index} value={item.name} />))}
            </datalist>
        ...
        <div className="crypto-table">
          ...
          {coinsDisplay.slice(0, 10).map((item, index) => (
            <Link to={`coin/${item.id}`} className="table-layout" key={index}>
              ...
              <div className="coin-name"><img src={item.image} alt="coin image" />{item.name} - {item.symbol}</div>

              <div><p>{currency.symbol}&nbsp;&nbsp; {item.current_price.toLocaleString()} </p></div>

              <div><p id="change24" style={{color:item.price_change_percentage_24h > 0 ? "#00d515" : "#ff4646",}}> {Math.floor(item.price_change_percentage_24h * 100) / 100}</p></div>
